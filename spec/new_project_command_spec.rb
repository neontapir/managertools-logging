...
RSpec.describe NewProjectCommand do
  ...
  context 'when creating a new project' do
    subject(:bloodties) { Project.find('Bloodties') }

    let(:bloodties_log) { File.join %W[#{ProjectFolder.root} bloodties #{Settings.log_filename}] }
    ...
    it 'finds the newly created project' do
      expect(bloodties).not_to be_nil
    end

    it 'creates the log in the expected path' do
      expect(bloodties.file.path).to eq bloodties_log
      expect(File).to exist bloodties_log
    end

    it 'created the log with the expected contents' do
