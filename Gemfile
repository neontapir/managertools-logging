# frozen_string_literal: true

source 'https://rubygems.org'

gem 'asciidoctor', '~> 1.5.6'
gem 'command', '~> 1.0'
gem 'configatron', '~> 4.5'
gem 'facets', '~> 3.1'
gem 'highline', '~> 1.7'
gem 'rake', '~> 10.5'
gem 'settingslogic', '~> 2.0', '>= 2.0.9'
gem 'trollop', '~> 2.1'
gem 'unicode', '~> 0.4', '>= 0.4.4'

gem 'yard', '~> 0.9', '>= 0.9.9', require: false, group: :docs

gem 'flay', '~> 2.10', require: false, group: :quality
gem 'flog', '~> 4.6', require: false, group: :quality
gem 'reek', '~> 4.7', require: false, group: :quality

gem 'mutant-rspec', '0.8.11', require: false, group: :test
gem 'rspec', '~> 3.5', require: false, group: :test
gem 'rubocop', '~> 0.50', require: false, group: :test
gem 'should_not', '~> 1.1', require: false, group: :test
gem 'simplecov', '~> 0.15', require: false, group: :test
gem 'timecop', '~> 0.9', require: false, group: :test
